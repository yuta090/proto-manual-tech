...
1. **タスクリストをドキュメントとして保存:** タスクリスト作成後、個人開発ではissue管理システムを使わないことが多いため、以下のようなプロンプトでタスクリストをドキュメントファイルとして保存しておきましょう。

```
個人での開発なので、issueは使わずに、タスクリストをドキュメントとして保存し、セッションが切れても参照できるようにしてください。
```

これにより、Super Claudeがタスクリストを `tasks.md` や `TODO.md` などのファイルに保存してくれます。セッションを跨いでも進捗状況を確認でき、どのタスクが完了したかを追跡できます。
...
1. **動作確認とE2Eテスト:** 生成された`index.html`ファイルをブラウザで開くなどして、フォームが正しく表示され、データが保存されるか**目視で確認**します。目視確認が完了したら、AIにE2Eテストを依頼して品質を確保しましょう。以下のプロンプトを入力します。

```
/sc:test 今作成した内容をe2eテストしたい。問題なければタスク完了。次のタスクへ
```

このプロンプトにより、Super Claudeが自動的にE2Eテストを実行し、問題がなければタスク完了として次のタスクに進むことができます。この「実装→目視確認→E2Eテスト→次のタスク」のサイクルを繰り返すことで、品質を保ちながら確実にアプリが形になっていきます。
