...
function Register-ClaudePro {
    ...
    if ($response -notmatch '^[Yy]$') {
        Write-Warning-Custom "Claude Pro の登録は後で行えます（推奨）"
    ...
    while ($true) {
        ...
        if ($confirm -match '^[Yy]$') {
            ...
            $skip = Read-Host "このステップを一旦スキップしますか? (y/N)"
            if ($skip -match '^[Yy]$') {
                Write-Warning-Custom "Claude Pro の登録は未完了です。必要になったら https://claude.ai/upgrade で登録してください"
                return
            }
        }
    }
...
function Main {
    ...
    if (-not $isAdmin) {
        Write-Error-Custom "このスクリプトを実行するには管理者権限が必要です"
        Write-Info "PowerShell を右クリックし『管理者として実行』で再実行してください"
        exit 1
    }
