...
function Install-GitHubCLIFromMsi {
    Write-Info "winget が利用できないため、MSI パッケージで GitHub CLI をインストールします..."

    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    } catch {
        Write-Warn "TLS 設定の更新に失敗しました: $($_.Exception.Message)"
    }
    ...
    try {
        ...
        if (-not $release.assets) {
            Write-Err "GitHub CLI のリリース情報を取得できませんでした"
            return $false
        }
        ...
        if (-not $asset) {
            Write-Err "対応するアーキテクチャ用の GitHub CLI MSI が見つかりませんでした (${archSuffix})"
            return $false
        }
        ...
        if ($process.ExitCode -ne 0) {
            Write-Err "MSI インストーラがエラーコード $($process.ExitCode) を返しました"
            return $false
        }
        ...
        Write-Err "MSI インストール後も GitHub CLI が見つかりません"
        return $false
    } catch {
        Write-Err "GitHub CLI の MSI インストールに失敗しました: $($_.Exception.Message)"
        return $false
    } finally {
        ...
        if (Get-Command gh -ErrorAction SilentlyContinue) {
            ...
            if (Get-Command winget -ErrorAction SilentlyContinue) {
                ...
                try {
                    ...
                    Write-Warn "winget でのインストールに失敗しました: $($_.Exception.Message)"
                }
                ...
                Write-Warn "winget コマンドが見つかりません。MSI でのインストールに切り替えます"
            }
            ...
            if (-not $installed) {
                Write-Err "GitHub CLI のインストールに失敗しました。App Installer (winget) の更新やネットワーク設定を確認してください"
                exit 1
            }
        }
